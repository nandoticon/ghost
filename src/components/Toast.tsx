            ...
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] flex flex-col items-center space-y-2 pointer-events-none">
                {toasts.map(toast => (
                    <div
                        key={toast.id}
                        className={cn(
                            "flex items-center space-x-3 px-6 py-3 rounded-2xl shadow-2xl border backdrop-blur-xl animate-in slide-in-from-bottom duration-300 pointer-events-auto max-w-[90vw] md:max-w-md",
                            ...
                            toast.type === 'info' && "bg-surface-secondary/80 border-border text-text-primary"
                        )}
                    >
